  ...
  useEffect(() => {
    ...
    switch (activeTab) {
      ...
      case 'new':
        ...
        filtered = filtered.filter(customer => 
          new Date(customer.firstService) >= thirtyDaysAgo
        );
        break;
      default:
  ...
  const fetchCustomers = async () => {
    try {
      ...
      appointments?.forEach(appointment => {
        ...
        if (new Date(appointment.preferred_date) < new Date(customer.firstService)) {
          customer.firstService = appointment.preferred_date;
        }
        if (new Date(appointment.preferred_date) > new Date(customer.lastService)) {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: colors.primary, fontSize: '1.1rem' }}>Loading Customers...</p>
        </div>
      </div>
    );
  }
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            {[
              ...
              { key: 'frequent', label: 'Regulars', count: customers.filter(c => c.totalServices >= 3).length },
              { key: 'new', label: 'New (30 days)', count: customers.filter(c => {
                const thirtyDaysAgo = new Date();
                thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
              ...
              >
                {tab.label}
                <span style={{
                  backgroundColor: activeTab === tab.key ? colors.background : colors.surfaceDark,
              ...
              {filteredCustomers.length === 0 ? (
                <div style={{
                  ...
                  <p style={{ color: colors.textSecondary, margin: 0 }}>
                    {searchQuery ? 'Try adjusting your search terms' : 'No customer data available'}
                  </p>
                </div>
              ) : (
                <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{
                            ...
                            <span style={{
                              backgroundColor: getLoyaltyLevel(customer.totalServices).bgColor,
                              color: getLoyaltyLevel(customer.totalServices).color,
                              padding: '0.25rem 0.75rem',
                              borderRadius: '20px',
                              fontSize: '0.75rem',
                              fontWeight: '600'
                ...
                <div style={{ marginBottom: '2rem' }}>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ fontSize: '1rem', fontWeight: '600', color: getLoyaltyLevel(selectedCustomer.totalServices).color }}>
